...
class dataset(object):
    ...
    def record_analysis_metrics(self, split, num_folds, kfold_mse, kfold_mae, test_mse, test_mae, dataset_index=None):
    ...
    def analysis_dict(self, split, k, kfold_mse, kfold_mae, test_mse, test_mae):
        return { 'split': split, 'k': k, 'kfold_mse': kfold_mse, 'kfold_mae': kfold_mae, 'test_mse': test_mse, 'test_mae': test_mae}
...
def euclidean_distance(v1, v2):
    if len(v1) != len(v2):
        raise ValueError:
            print "In euclidean_distance: Length of vector v1 and v2 do not match\n", "len(v1): ", len(v1), '\tlen(v2): ', len(v2)
...
def main():
    ...
    for i, j in t_v_generator:
        ...
        mse, mae = test_prediction(X, Theta, validation_df)

        k_fold_mse.insert(len(k_fold_mse), mse)
        k_fold_mae.insert(len(k_fold_mse), mae)
    ...
    d.record_analysis_metrics(split="random_sample_split", num_folds=len(k_fold_mse), kfold_mse=k_fold_mse, kfold_mae=k_fold_mae, test_mse=test_mse, test_mae=test_mae)

    print d.analysis_sets[index]

    # with open('./records.txt', 'a') as f:
    #     s = "\n"+str(num_features)+"\t"+str(lamb)+"\t"+str(mse)
    #     f.write(s)
